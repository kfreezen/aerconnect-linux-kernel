 ...
 *
 * Original driver file taken from: https://android.googlesource.com/kernel/msm/+/android-msm-angler-3.10-oreo-r6/drivers/input/touchscreen/ft5x06_ts.c
 * Stripped out references to sensors.h
 */
...
/*
 * This FT_55_AA_DLY_NS seems to indicate that the delay here is supposed to be 5 microseconds. 
 * However, the define was used in usleep() and usleep's unit is the microsecond.
 *
 */
#define FT_55_AA_DLY_NS		5000
#define FT_55_AA_DLY_MAX_VARY 1000
			...
			const u8 *data, u32 data_len)
{
	...
	for (i = 0, j = 0; i < FT_UPGRADE_LOOP; i++) {
		...
		usleep_range(FT_55_AA_DLY_NS, FT_55_AA_DLY_NS + FT_55_AA_DLY_MAX_VARY);
...
[focaltech,panel-coords | focaltech,display-coords] = <x1 y1 x2 y2>;
focaltech,i2c-pull-up = <0|1>; (optional)
focaltech,no-force-update = <0|1>; (optional)
focaltech,reset-gpio = <&gpioX pin GPIO_ACTIVE_LOW>;
focaltech,irq-gpio = <&gpio2 25 IRQ_TYPE_EDGE_FALLING>;
focaltech,fw-name = "ft_fw.bin"; (optional)
			...
			struct ft5x06_ts_platform_data *pdata)
{
	...
	if (rc) {
		dev_err(dev, "Unable to read display coords (or panel coords?)");
		return rc;
	}
	...
	if (pdata->reset_gpio < 0) {
		dev_err(dev, "Failed to read focaltech,reset-gpio.");
	...
	if (pdata->irq_gpio < 0) {
		dev_err(dev, "Failed to read focaltech,irq-gpio.");
	...
	if (!rc) {
		...
		dev_err(dev, "Unable to read group-id");
		return rc;
	}
	...
	if (!rc) {
		...
		dev_err(dev, "Unable to read hard-reset-delay-ms");
		return rc;
	}
	...
	if (!rc) {
		...
		dev_err(dev, "Unable to read soft-reset-delay-ms");
		return rc;
	}
	...
	if (!rc) {
		...
		dev_err(dev, "Unable to read num-max-touches");
		return rc;
	}
	...
	if (!rc) {
		...
		dev_err(dev, "Failed to read family-id.");
		return rc;
	}
			   ...
			   const struct i2c_device_id *id)
{
	...
	*/
	...
	if (err) {
		...
		dev_err(&client->dev, "requested irq: %d\n", client->irq);
	}
